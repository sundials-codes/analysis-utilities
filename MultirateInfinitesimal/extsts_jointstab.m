function [xgrid,ygrid,Rmax] = extsts_jointstab(G,W,dc,maxtheta,maxrho,numRay,numAngle,box,numGrid,plottype)
  % Figure out the combined stability given A(theta) reaction stability region
  % and A(0) diffusion stability region
  % G cell array with gamma matrices
  % W cell array with omega matrices
  % dc differences between c values
  % maxtheta is the maximum theta angle to test
  % maxrho is the maximum radius to test the A(theta) and A(0) regions
  % numRay is the number of points along each ray emanating from the origin
  % numAngle is the number of angles sampled in in the [-maxtheta,maxtheta] sector
  % box = [xl xr yb yt] defines x and y limits of the joint stability plotting region
  %       assumes yt = -yb symmetry about the real axis
  % numGrid is the number of points taken along each direction (x or y) in the grid
  % plottype is one of {explicit, implicit, imex, explicit_mrigark, implicit_mrigark, imexmri},
  %   corresponding to the type of method that should be plotted.
  %
  % Daniel R. Reynolds
